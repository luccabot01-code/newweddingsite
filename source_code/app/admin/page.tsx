...
import { Plus, LayoutTemplate, ArrowRight, ExternalLink, Settings, Loader2, Trash2, Users } from "lucide-react";
...
export default function AdminHub() {
    ...
    async function handleCreate(e: React.FormEvent) {
        ...
        // Server handles the default data loading now
    ...
    async function handleDelete() {
        ...
        setDeleteModalOpen(false); // Close immediately for better UX or keep open with loading state
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    ...
                    >
                        ...
                        <p className="text-stone-400 text-center text-sm mb-6">
                            Are you sure you want to delete <span className="text-white font-mono bg-white/5 px-1 rounded">/{eventToDelete}</span>? This action cannot be undone.
                        </p>
                ...
                <header className="mb-20 flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div className="space-y-4">
                        ...
                        <p className="text-stone-400 text-lg max-w-xl leading-relaxed">
                            Create and manage multiple wedding websites from a single control center. Each event gets a unique link and dashboard.
                        </p>
                    </div>
                            ...
                            >
                                <div className="flex-1">
                                    <div className="flex items-start justify-between mb-6">
                                        ...
                                        <span className={`px-3 py-1 rounded-full text-xs font-medium border ${new Date() > new Date(event.date) ? 'bg-red-500/10 text-red-500 border-red-500/10' : 'bg-green-500/10 text-green-500 border-green-500/10'}`}>
                                ...
                                <div className="grid grid-cols-2 gap-3 mt-auto mb-4">
                                    <a
                                        ...
                                        className="flex items-center justify-center gap-2 px-4 py-3 bg-white text-black rounded-xl font-medium hover:bg-stone-200 transition-colors text-sm"
                                    >
                                    ...
                                    <a
                                        ...
                                        className="flex items-center justify-center gap-2 px-4 py-3 bg-stone-800 text-stone-200 rounded-xl font-medium hover:bg-stone-700 transition-colors text-sm"
                                    >
                                    ...
                                    <a
                                        href={`/${event.slug}`}
                                        target="_blank"
                                        className="col-span-2 flex items-center justify-center gap-2 px-4 py-3 border border-white/10 text-stone-400 rounded-xl font-medium hover:bg-white/5 hover:text-white transition-colors text-sm group-hover:border-white/20"
                                    >
